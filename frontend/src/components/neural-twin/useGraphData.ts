  ...
  try {
    ...
    for (const acct of accountList) {
      ...
      nodes.push({
        ...
        size: Math.max(2, Math.min(8, Math.abs(balance) / 500000)),
        pulse: false,
    ...
    if (cashflow) {
      nodes.push({
        ...
        size: 3 + (cashflow.confidence || 0.5) * 2,
        pulse: false,
        glow: false,
    ...
    if (compliance) {
      nodes.push({
        ...
        size: 2.5 + (compliance.overall_score || 0.5) * 2,
        pulse: false,
    ...
    Object.entries(categoryGroups).forEach(([cat, data], i) => {
      ...
      nodes.push({
        ...
        vx: 0, vy: 0, vz: 0,
        color: '#8b5cf6',
        size: Math.min(6, Math.max(1.5, data.total / 100000)),
        pulse: false,
        glow: false,
